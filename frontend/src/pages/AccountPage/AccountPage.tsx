...
import { ShortInfoList } from '@/modules/Profile/components/ShortInfoList/ShortInfoList';
  ...
  useEffect(() => {
    ...
    const loadUser = async () => {
      if (Number(user_id) !== profileStore.user.user_id) {
        try {
          const fetchedUser = await profileStore.GetUserDataById(Number(user_id));
          setUser(fetchedUser);
        } catch (e) {
  ...
  return (
    <div>
      <div className={styles.info}>
        ...
        <ShortInfoList user_id={Number(user_id)} />
      </div>
    </div>
  );
});
